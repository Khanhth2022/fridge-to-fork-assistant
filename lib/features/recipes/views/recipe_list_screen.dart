...
class _RecipeListScreenState extends State<RecipeListScreen> {
	...
	Widget build(BuildContext context) {
		return Scaffold(
			appBar: AppBar(
				title: const Text('Goi y mon an AI'),
			),
				...
				builder: (BuildContext context, _) {
					return Column(
						children: <Widget>[
							...
							if (_viewModel.isLoading) const LinearProgressIndicator(minHeight: 2),
							Expanded(
	...
	Widget _buildContent(BuildContext context) {
		...
		if (_viewModel.recipes.isEmpty) {
			return const Center(
				child: Text('Khong tim thay cong thuc phu hop voi nguyen lieu hien tai.'),
			);
		}
									...
									Wrap(
										...
										children: <Widget>[
											if (recipe.readyInMinutes != null)
												_MetaChip(icon: Icons.schedule, text: '${recipe.readyInMinutes} phut'),
											if (recipe.servings != null)
												_MetaChip(icon: Icons.people, text: '${recipe.servings} khau phan'),
