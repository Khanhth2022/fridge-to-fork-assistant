...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> {
	...
	void initState() {
		...
		_viewModel = widget.viewModel ?? RecipeViewModel(apiClient: RecipeApiClient());
	...
	Widget build(BuildContext context) {
		return Scaffold(
			appBar: AppBar(
				title: Text(widget.recipeTitle ?? 'Chi tiet mon an'),
			),
			body: AnimatedBuilder(
				...
				builder: (BuildContext context, _) {
					...
					if (recipe == null) {
						return const Center(child: Text('Khong tim thay thong tin mon an.'));
					}
										...
										children: <Widget>[
											...
											Wrap(
												...
												children: <Widget>[
													...
													if (recipe.isVegan)
														const _DetailChip(icon: Icons.energy_savings_leaf, text: 'Vegan'),
													if (recipe.isVegetarian && !recipe.isVegan)
														const _DetailChip(icon: Icons.eco, text: 'Vegetarian'),
												],
											),
											const SizedBox(height: 16),
											_InfoSection(
												title: 'Tom tat',
												child: Text(_stripHtmlTags(recipe.summary ?? 'Dang cap nhat...')),
											),
											const SizedBox(height: 16),
											_InfoSection(
												title: 'Nguyen lieu',
											...
											_InfoSection(
												title: 'Huong dan',
												child: Text(
													_stripHtmlTags(recipe.instructions ?? 'Dang cap nhat huong dan nau an.'),
												),
											),
											if (recipe.sourceUrl != null && recipe.sourceUrl!.isNotEmpty) ...<Widget>[
												const SizedBox(height: 16),
												_InfoSection(
