...
class RecipeViewModel extends ChangeNotifier {
	RecipeViewModel({required RecipeApiClient apiClient}) : _apiClient = apiClient;
	...
	List<String> get mockPantryIngredients => List<String>.unmodifiable(_mockPantryIngredients);
	...
	Future<void> fetchRecipes({List<String>? pantryIngredients}) async {
		...
		try {
			final List<String> ingredients = pantryIngredients ?? _mockPantryIngredients;
	...
	void updateMockPantryIngredients(List<String> ingredients) {
		_mockPantryIngredients
			..clear()
			..addAll(ingredients.where((String item) => item.trim().isNotEmpty).map((e) => e.trim()));
		notifyListeners();
	}
